...
访问 `/product-copy`，粘贴你有权使用的公开商品链接，可提取页面元数据并编辑标题、描述、关键词和类目 ID，再保存到浏览器本机草稿箱。集合页同步后，可勾选商品批量替换商标/标题/描述/关键词/类目/价格/MOQ，并通过兼容草稿接口创建阿里国际站草稿；当前不会自动复制受保护素材，也不会自动上架。若接口返回类目或 Schema 校验错误，商品会保留在 MEEKA 并标记为待检查/失败。
